= 2.0 (unreleased)
* improved performance of <tt>has_all_roles?</tt> method using subselect instead of doing one DB lookup per argument in worst case
* clean up the initializer code, detecting if it's loaded by Rails::Server or Rails::Console
* now also running on Rubinius, REE and Ruby 1.8
