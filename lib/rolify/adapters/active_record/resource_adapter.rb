  ...
  module Adapter
    class ResourceAdapter < ResourceAdapterBase
      ...
      def in(relation, user, role_names)
        ...
        relation.where("#{quote_table(role_class.table_name)}.#{quote_column(role_class.primary_key)} IN (?) AND ((#{quote_table(role_class.table_name)}.resource_id = #{quote_table(relation.table_name)}.#{quote_column(relation.primary_key)}) #{"OR (#{quote_table(role_class.table_name)}.resource_id = #{quote_table(relation.table_name)}.#{Rolify.resource_primary_key})" if column_exists?(quote_table(relation.table_name), Rolify.resource_primary_key)} OR (#{quote_table(role_class.table_name)}.resource_id IS NULL))", roles)
      end
